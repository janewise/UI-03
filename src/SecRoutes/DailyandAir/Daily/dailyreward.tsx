...
const random = Math.floor(Math.random() * (10000 - 5000 + 1)) + 5000; // Generates a random number between 500 and 800
const dailyRewards = [1000, random, 3000, 4000, 5000, 6000, 7000, 8000, 9000, 15000];
...
const Dailyreward: React.FC<DailyrewardProps> = ({ balanceRef, onRewardClaimed }) => {
  ...
  useEffect(() => {
    ...
    if (storedDay && storedTimestamp && storedClaimableDay && storedHasClaimed) {
      ...
      if (timeDifference > 10 * 1000 && timeDifference <= 120 * 1000 && hasClaimed) { // 1 minute to 2 minutes
  ...
  return (
    <div>
      ...
      <div
    className={`card ${1 === claimableDay && !hasClaimed && !initialLoad ? 'active' : ''}`}
      ...
      <div
        className={`card ${3 === claimableDay && !hasClaimed && !initialLoad ? 'active' : ''}`}
      ...
      <div
      className={`card ${4 === claimableDay && !hasClaimed && !initialLoad ? 'active' : ''}`}
      ...
      <div
     className={`card ${5 === claimableDay && !hasClaimed && !initialLoad ? 'active' : ''}`}
      ...
      <div
       className={`card ${6 === claimableDay && !hasClaimed && !initialLoad ? 'active' : ''}`}
      ...
      <div
       className={`card ${7 === claimableDay && !hasClaimed && !initialLoad ? 'active' : ''}`}
      ...
      <div
       className={`card ${8 === claimableDay && !hasClaimed && !initialLoad ? 'active' : ''}`}
      ...
      <div
     className={`card ${9 === claimableDay && !hasClaimed && !initialLoad ? 'active' : ''}`}
      ...
      <div
      className={`card ${10 === claimableDay && !hasClaimed && !initialLoad ? 'active' : ''}`}
      ...
      <div
  className={`card ${2 === claimableDay && !hasClaimed && !initialLoad ? 'active' : ''}`}
